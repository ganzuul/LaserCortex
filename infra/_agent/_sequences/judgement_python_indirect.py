...
def set_up_judgement_python_indirect_demo(agent: AgentFrame):
    logger.debug("Setting up judgement_python_indirect demo sequence")
    working_interpretation = agent.working_interpretation
    body = agent.body

    @register_inference_sequence("judgement_python_indirect")
    def judgement_python_indirect(self: Inference):
        """`(IWI-IR-MFP-MVP-TVA-TIP-MIA-OR-OWI)`"""
        logger.info("=====EXECUTING JUDGEMENT PYTHON INDIRECT SEQUENCE=====")
        states = States()
        ...
        logger.info("=====JUDGEMENT PYTHON INDIRECT SEQUENCE COMPLETED=====")
        return states

def configure_judgement_python_indirect_demo(agent: AgentFrame, inference_instance: Inference, methods: dict[str, Callable]):
    logger.debug("Configuring judgement_python_indirect demo steps")
