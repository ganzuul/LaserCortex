...
def set_up_imperative_input_demo(agent_frame):
    logger.debug("Setting up imperative_input demo sequence")
    ...
    @register_inference_sequence("imperative_input")
    def imperative_input(self: Inference):
        """`(IWI-IR-MFP-MVP-TVA-TIP-MIA-OR-OWI)`"""
        logger.info("=====EXECUTING IMPERATIVE INPUT SEQUENCE=====")
        ...
        logger.info("=====IMPERATIVE INPUT SEQUENCE COMPLETED=====")
        return states

def configure_imperative_input_demo(agent_frame, inference_instance: Inference, methods: dict[str, Callable]):
    logger.debug("Configuring imperative_input demo steps")
