...
from infra._states._judgement_direct_states import States as JudgementDirectStates
import logging
...
def set_up_judgement_direct_demo(agent_frame):
    logger.debug("Setting up judgement_direct demo sequence")
    ...
    @register_inference_sequence("judgement_direct")
    def judgement_direct(self: Inference):
        """`(IWI-IR-MFP-MVP-TVA-TIP-MIA-OR-OWI)`"""
        logger.info("=====EXECUTING JUDGEMENT DIRECT SEQUENCE=====")
        states = JudgementDirectStates()
        ...
        logger.info("=====JUDGEMENT DIRECT SEQUENCE COMPLETED=====")
        return states

def configure_judgement_direct_demo(agent_frame, inference_instance: Inference, methods: dict[str, Callable]):
    logger.debug("Configuring judgement_direct demo steps")
