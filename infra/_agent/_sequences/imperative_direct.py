...
def set_up_imperative_direct_demo(agent_frame):
    logger.debug("Setting up imperative_direct demo sequence")
    ...
    @register_inference_sequence("imperative_direct")
    def imperative_direct(self: Inference):
        """`(IWI-IR-MFP-MVP-TVA-TIP-MIA-OR-OWI)`"""
        logger.info("=====EXECUTING IMPERATIVE DIRECT SEQUENCE=====")
        ...
        logger.info("=====IMPERATIVE DIRECT SEQUENCE COMPLETED=====")
        return states

def configure_imperative_direct_demo(agent_frame, inference_instance: Inference, methods: dict[str, Callable]):
    logger.debug("Configuring imperative_direct demo steps")
