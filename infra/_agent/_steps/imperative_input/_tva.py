...
from infra._core import Reference, cross_action
...
def tool_value_actuation(states: States) -> States:
    """
    Apply the input function from MFP to the values from MVP.
    This replaces the LLM call in the standard imperative sequence.
    """
    ...
    if func_ref and values_ref:
        ...
        if func_callable and isinstance(func_callable, Callable):
            # Wrap the input function to ensure its output is a list,
    ...
    logging.debug(f"TVA completed. User input received and stored in inference state: {states.inference}")
    return states