...
\section{Meta-Paradox: The Sorry as a First-Class Problem}
...
The \textsf{sorry} in \textsf{fuzzyLiar.proof} IS the evidence of this meta-paradox.
It is not a placeholder to be filled later --- it is the manifestation of the
framework's current incompleteness, stated as a first-class Problem.
\end{definition}

\begin{definition}[Fuzzy Missing Proof]
\label{def:fuzzy_missing}
\lean{LiarParadox.fuzzyMissingProof}
\leanok
\uses{def:metaparadox, def:fuzzy_liar}
The meta-paradox arising from the missing Fuzzy \textsf{LogicContraction}.
Its resolution is to fill the \textsf{sorry} in \textsf{LogicTypes.LogicContraction(.Fuzzy)}.
\end{definition}

\begin{definition}[All Missing Contractions]
\label{def:missing_all}
\lean{LiarParadox.missingContractions}
\leanok
\uses{def:metaparadox}
The list of 11 meta-paradoxes, one for each non-Classical logic type
whose \textsf{LogicContraction} is undefined.
Each is a concrete blocker for the Very Big Box product.
\end{definition}
...
\subsection{Meta-Paradoxes in \textsf{LiarParadox.lean}}

\begin{itemize}
  \item \textbf{\textsf{fuzzyLiar.proof}} --- a $\mathsf{sorry}$; blocked by the missing
    Fuzzy \textsf{LogicContraction}. This sorry is formalized as a first-class
    meta-paradox in \textsf{fuzzyMissingProof}.
  \item \textbf{\textsf{missingContractions}} --- the 11 meta-paradoxes arising from
    all non-Classical missing \textsf{LogicContraction} definitions.
    Each is a concrete cell in the Very Big Box product that cannot yet be filled.
  ...
  \item \textbf{Other problem classes} --- only $\mathsf{selfReference}$ and $\mathsf{metaParadox}$
    are instantiated as \textsf{Problem}s. The other 10 classes have problems
