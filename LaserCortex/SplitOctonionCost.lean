...
projects onto the `NodeCost` cost algebra. Uses only core Lean (`Int`, `Nat`,
`Init`, `by decide`), with no Mathlib dependency.
...
#lean4-theorem #split-octonion #engine #proof-bound #dynamical-system
-/
