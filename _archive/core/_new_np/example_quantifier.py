...
if __name__ == "__main__":
    ...
    <= *every({digit position})%:[{digit position}].[{digit pairs}?]^({digit position}*)@1
        <- [{number 1 digit} and {number 2 digit}]
            <= *every({two numbers})%:[{two numbers}].[{digit in position}?]^({position}*)@2
            <- {digit in position}*1
            <- {digit in position}*2
        <- {two numbers}
        <- {digit pairs}*?
